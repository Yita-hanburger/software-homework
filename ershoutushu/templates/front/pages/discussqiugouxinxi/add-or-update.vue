	...
	<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","textAlign":"center","fontWeight":"500"}' class="title">评论内容</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussqiugouxinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussqiugouxinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussqiugouxinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discussqiugouxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussqiugouxinxiCleanType',true);
						var res1 = await this.$api.info('qiugouxinxi',this.ruleForm.refid)
						res1.data.discussnum++
						await this.$api.update('qiugouxinxi',res1.data)
