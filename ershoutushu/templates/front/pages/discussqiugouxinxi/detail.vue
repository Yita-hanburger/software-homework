...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussqiugouxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussqiugouxinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussqiugouxinxi', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussqiugouxinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussqiugouxinxi');
			...
			async init(){
				...
				let res = await this.$api.info('discussqiugouxinxi', this.id);
				this.detail = res.data;
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussqiugouxinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussqiugouxinxi/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussqiugouxinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussqiugouxinxi',that.detail)
							setTimeout(()=>{
