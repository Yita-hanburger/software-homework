		...
		<view :style='{"width":"100%","padding":"0 0 40rpx","position":"relative","background":"#dfd8cc","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#fff","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #fafafa","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.ziyuanmingcheng" type="text" placeholder="资源名称" ></input>
				</view>
			...
			<view :style='{"width":"100%","background":"#fff","height":"auto"}'>
				<scroll-view scroll-x="true" class="category-one" :style='{"padding":"10rpx ","whiteSpace":"nowrap","background":"#DFD8CC","display":"flex","width":"100%","justifyContent":"space-around","height":"auto"}'>
					<view :class='categoryName === item.ziyuanfenlei ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.ziyuanfenlei)">{{item.ziyuanfenlei}}</view>
				</scroll-view>
			...
			<view class="list" :style='{"width":"100%","padding":"24rpx","margin":"0","background":"#DFD8CC","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 4rpx rgba(0,0,0,.3)","padding":"20rpx","margin":"0 0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#FFFFFF","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"260rpx","objectFit":"cover","borderRadius":"10rpx","display":"block","height":"200rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.ziyuantupian)" :src="product.ziyuantupian.split(',')[0]"></image>
					<image :style='{"width":"260rpx","objectFit":"cover","borderRadius":"10rpx","display":"block","height":"200rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.ziyuantupian?baseUrl+product.ziyuantupian.split(',')[0]:''"></image>

					<view class="listmr" :style='{"width":"calc(100% - 260rpx)","padding":"0","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
						<view class="col3 f30 elip mb15" :style='{"width":"100%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"28rpx","color":"#000000"}'>{{product.ziyuanmingcheng}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('ziyuanfenxiang','修改')) || (!userid && isAuthFront('ziyuanfenxiang','修改'))" @click.stop="onUpdateTap(product)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('ziyuanfenxiang','删除')) || (!userid && isAuthFront('ziyuanfenxiang','删除'))" @click.stop="onDeleteTap(product.id)">
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('ziyuanfenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('ziyuanfenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,ziyuanfenlei:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.ziyuanfenlei = '%' + this.categoryName + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`ziyuanfenxiang`, params);
                } else {
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('ziyuanfenxiang', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`ziyuanfenxiang`, searchForm);
                } else {
                    res = await this.$api.list(`ziyuanfenxiang`, searchForm);
                }
				// 如果是第一页数据置空
