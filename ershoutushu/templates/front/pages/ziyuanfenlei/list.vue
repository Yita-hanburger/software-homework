		...
		<view :style='{"width":"100%","padding":"0 0 40rpx","position":"relative","background":"#dfd8cc","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#fff","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #fafafa","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.ziyuanfenlei" type="text" placeholder="资源分类" ></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"24rpx","margin":"0","background":"#DFD8CC","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 4rpx rgba(0,0,0,.3)","padding":"20rpx","margin":"0 0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#FFFFFF","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('ziyuanfenlei','修改')) || (!userid && isAuthFront('ziyuanfenlei','修改'))" @click.stop="onUpdateTap(product)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('ziyuanfenlei','删除')) || (!userid && isAuthFront('ziyuanfenlei','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.ziyuanfenlei){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`ziyuanfenlei`, params);
                } else {
                    res = await this.$api.list(`ziyuanfenlei`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('ziyuanfenlei', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.ziyuanfenlei){
					searchForm['ziyuanfenlei'] = '%' + this.searchForm.ziyuanfenlei + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`ziyuanfenlei`, searchForm);
                } else {
                    res = await this.$api.list(`ziyuanfenlei`, searchForm);
                }
				// 如果是第一页数据置空
